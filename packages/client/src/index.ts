...
export class OpenGEOClient {
  ...
  async capabilities<T = unknown>(): Promise<T> { const r = await this.request(`${this.options.baseUrl}/v1/capabilities`, { headers: this.headers() }); if (!r.ok) throw new Error(`OpenGEO request failed: ${r.status}`); return r.json() as Promise<T>; }
  async getJob<T = unknown>(id: string): Promise<T> { const r = await this.request(`${this.options.baseUrl}/v1/jobs/${encodeURIComponent(id)}`, { headers: this.headers() }); if (!r.ok) throw new Error(`OpenGEO request failed: ${r.status}`); return r.json() as Promise<T>; }
