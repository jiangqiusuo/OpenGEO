...
describe('Community client',()=>{
  it('creates a neutral overseas monitoring Job request without provider fields',async()=>{
    ...
    await expect(client.createMonitorRun({prompts:['Where is OpenGEO mentioned?'],monitoring_profile:'global-llm-v1',turnaround_class:'interactive'},'idem-1',1500)).resolves.toMatchObject({id:'job_demo_queued'});
